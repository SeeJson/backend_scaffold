...
func main() {
	...
	flag.StringVar(&inputFile, "input", "./api_spec.yaml", "")
	//flag.StringVar(&inputFile, "input", "../../cmd/backend/api_spec.yaml", "")
	...
	spec, err := apispec.LoadFile(inputFile)
	Panic(err)

	pp.Println(spec)

	sdm, err := docgen.ApiSpec2ShowDoc(spec)
	Panic(err)
